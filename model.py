...
class ResBlock(nn.Module):
    def __init__(self, args, downsample=True, rescale=True, filters=64, \
        ...
        # self.bn1 = InstanceNorm(filters, affine=True)
        # self.bn1 = nn.InstanceNorm2d(filters, affine=True)
        ...
        # self.bn2 = InstanceNorm(filters, affine=True)
        # self.bn2 = nn.InstanceNorm2d(filters, affine=True)
        # self.bn2 = nn.GroupNorm(32, filters, affine=True)
...
class ResNetModel(nn.Module):
    def __init__(self, args, stride=2):
        ...
        self.res_1b = ResBlock(args, filters=filter_dim, rescale=True, spec_norm=self.spec_norm, norm=self.norm)

        self.res_2a = ResBlock(args, filters=2*filter_dim, downsample=False, spec_norm=self.spec_norm, norm=self.norm)
        self.res_2b = ResBlock(args, filters=2*filter_dim, rescale=True, spec_norm=self.spec_norm, norm=self.norm)

        self.res_3a = ResBlock(args, filters=4*filter_dim, downsample=False, spec_norm=self.spec_norm, norm=self.norm)
        self.res_3b = ResBlock(args, filters=4*filter_dim, rescale=True, spec_norm=self.spec_norm, norm=self.norm)

        self.res_4a = ResBlock(args, filters=5*filter_dim, downsample=False, spec_norm=self.spec_norm, norm=self.norm)
        self.res_4b = ResBlock(args, filters=5*filter_dim, downsample=False, rescale=False, spec_norm=self.spec_norm, norm=self.norm)
